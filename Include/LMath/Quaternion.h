...
public:
	...
	{
		return acos(fmin(fabs(DotProduct(rhs)), L_One)) * L_Two;
	}
	...
	{
		ElementType dot = fromVector.DotProduct(toVector);
		...
		return QuaternionBase(cross.X(), cross.Y(), cross.Z(), dot + k).Normalize();
	}

	QuaternionBase Inverse(const QuaternionBase& rotation) const
	{
		ElementType n = rotation.Norm();
		return rotation.Conjugate() / (n * n);
	}
	...
	{
		if (t < L_Zero) return Base::Normalize();
		else if (t > L_One) return b.Normalize();
		return LerpUnclamped(b, t);
	}

	QuaternionBase LerpUnclamped(const QuaternionBase& b, ElementType t) const
	{
		QuaternionBase quaternion;
		if (DotProduct(b) >= L_Zero)
	...
	{
		return LookRotation(forward, Vector3(L_Zero, L_One, L_Zero));
	}

	static QuaternionBase LookRotation(const Vector3& forward, const Vector3& upwards)
	{
		// Normalize inputs
		...
		if (L_One - fabs(forward.DotProduct(upwards)) < L_Epsilon)
	...
	QuaternionBase RotateTowards(QuaternionBase from, QuaternionBase to,
		ElementType maxRadiansDelta)
	{
		ElementType angle = Angle(from, to);
		if (angle == L_Zero)
			return to;
		maxRadiansDelta = fmax(maxRadiansDelta, angle - LMath::Constans::Pi);
		ElementType t = fmin(L_One, maxRadiansDelta / angle);
		return SlerpUnclamped(from, to, t);
	}

	QuaternionBase Slerp(QuaternionBase a, QuaternionBase b, ElementType t)
	{
	...
	QuaternionBase SlerpUnclamped(QuaternionBase a, QuaternionBase b, ElementType t)
	{
		ElementType n1;
		ElementType n2;
		ElementType n3 = Dot(a, b);
		bool flag = false;
		if (n3 < L_Zero)
		{
			flag = true;
	...
	void ToAngleAxis(QuaternionBase rotation, ElementType& angle, Vector3& axis)
	{
		if (rotation.W() > L_One)
	...
	Vector3 ToEuler() const
	{
		...
		ElementType unit = Base::NormSquared();// sqx + sqy + sqz + sqw;
	...
	{
		Vector3 u = Vector3(at(L_Zero), at(L_One), at(L_Two));
		ElementType s = Z();
		return u * (u.DotProduct(rhs)  * L_Two)
